...
$result = $conn->query("SELECT ib.id, b.title, b.author, ib.issue_date, ib.return_date, ib.status, ib.book_id
...
if (!$result) {
    die("<div style='padding:20px; background:#f8d7da; color:#721c24; border-radius:5px;'><strong>Database Error:</strong> " . htmlspecialchars($conn->error) . "</div>");
}

// Calculate statistics and total penalties
$statsQuery = $conn->query("SELECT 
    COUNT(*) as total_issued,
    SUM(CASE WHEN status='issued' THEN 1 ELSE 0 END) as currently_issued,
    SUM(CASE WHEN status='returned' THEN 1 ELSE 0 END) as returned_count
    FROM issued_books WHERE user_id=$user_id");
$stats = $statsQuery->fetch_assoc();

// Calculate total penalties
$today = date('Y-m-d');
$penaltiesQuery = $conn->query("SELECT 
    SUM(CASE WHEN status='issued' AND return_date < '$today' THEN CEIL(DATEDIFF('$today', return_date)) * 10 ELSE 0 END) as overdue_fine
    FROM issued_books WHERE user_id=$user_id");
$penalties = $penaltiesQuery->fetch_assoc();
$total_fine = $penalties['overdue_fine'] ?? 0;
?>
        ...
        <div class="card-body">
                    ...
                    <table class="table table-hover mb-0">
                        ...
                        <tbody>
                            <?php while($row = $result->fetch_assoc()): 
                                ...
                                $penalty = 0;
                                if ($is_overdue) {
                                    $days_late = ceil(($today - $row['return_date']) / 86400);
                            ...
                            ?>
                                <tr>
                                    ...
                                    <td>
                                        <small><?= date('M d, Y', strtotime($row['issue_date'])) ?></small>
                                    </td>
                                    <td>
                                        <small><?= date('M d, Y', strtotime($row['return_date'])) ?></small>
                                    </td>
                                    <td>
                                        <?php if ($row['status'] == 'issued'): ?>
                                            ...
                                            <?php else: ?>
                                                <span class="badge bg-warning">
                                                    <i class="fas fa-hourglass-half me-1"></i>Issued
                                                </span>
                                            <?php endif; ?>
                                        <?php else: ?>
                                    ...
                                    <td>
                                        <?php if ($row['status'] == 'issued'): ?>
                                            <a href="return.php?id=<?= $row['id'] ?>" class="btn btn-success btn-sm" title="Return this book">
            ...
            <?php else: ?>
                <div class="alert alert-info text-center">
                    ...
                    <p class="mb-0 mt-2">
                        <a href="books.php">Browse available books</a> to get started
                    </p>
                </div>
            <?php endif; ?>
        </div>
